...
import { createUserWithEmailAndPassword, signInWithEmailAndPassword, GoogleAuthProvider, signInWithPopup, sendPasswordResetEmail, sendEmailVerification, signOut } from 'firebase/auth';
  ...
  const handleGoogleSignIn = async () => {
      try {
          ...
          if (!userSnap.exists()) {
             ...
             const subId = localStorage.getItem('referralSub');
             let referredBy = null;
             let referredByUid = null;

             if (refCode) {
                const referrer = await getUserByAffiliateId(refCode);
                if (referrer) {
                    referredBy = refCode;
          ...
          if (adminEmail && userEmailLower === adminEmail) {
              try {
                  ...
                  // Use try-catch to allow login even if reading admins results in permission error
